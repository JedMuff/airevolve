...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--gate-cfg", choices=["backandforth", "figure8", "circle", "slalom"], default="figure8")
    ...
    p.add_argument("--resume-genomes", default=None)
    args = p.parse_args()
...
def _build_initial_population(args: argparse.Namespace, config: dict, WrappedHandler) -> tuple[list, dict | None]:
    if args.resume_genomes is not None:
        genomes_array = np.load(args.resume_genomes, allow_pickle=True)
        return list(genomes_array), None
    if args.init_pop_mode == "random":
        return generate_viable_initial_population(WrappedHandler(), args.population_size, is_indirect=False)
...
class _SnapshotCallback:
    ...
    def _save_pareto_csv(self, snap_dir: Path, population, generation: int) -> None:
        ...
        cols = ["id", "generation", "parent_ids", "rank", "crowding_distance", "gates_passed", "total_energy_j"]
        ...
        try:
            ...
            for _, row in front0.iterrows():
                ...
                try:
                    ...
                    viz.plot_3d(
                        genome_arr, ax=ax,
                        title=(f"Gen {generation} | id={ind_id} | gates={fitness[0]} | energy={fitness[1]:.0f}J"),
                        fitness=fitness[0],
                        generation=generation,
                    )
                    fig.savefig(str(snap_dir / f"morphology_{ind_id}.png"), dpi=150, bbox_inches="tight")
                    plt.close(fig)
                except Exception:
                    pass
        except Exception:
...
def _save_artefacts(all_individuals, results_dir: Path, args: argparse.Namespace) -> None:
    ...
    try:
        fig = plot_pareto_front(str(csv_path), title=f"{_EXPERIMENT_NAME}  (gen {last_gen})")
...
def main() -> None:
    ...
    if args.dry_run:
        return
    config = get_genome_handler_config(args.genome, args.min_narms, args.max_narms, num_segments=8, initial_hidden_nodes=0)
    ...
    def _patched_evaluate_population(fitness_function, population, ids, generation, all_parent_ids, log_dir_base, num_workers=1):
        if num_workers > 1:
            args_list = [(fitness_function, genome, ids[i], generation, all_parent_ids[i], log_dir_base) for i, genome in enumerate(population)]
            PER_INDIVIDUAL_TIMEOUT = 1800
            from functools import partial
            pool_init = partial(_standard_pool_worker_init, args.torch_threads)
            with NonDaemonPool(processes=min(num_workers, len(population)), initializer=pool_init) as pool:
                async_results = [pool.apply_async(_evo_components._evaluate_individual_worker, (a,)) for a in args_list]
                evaluated = []
                for i, ar in enumerate(async_results):
                    try:
                        ...
                        indiv_log_dir = os.path.join(gen_dir, f"individual_{ids[i]}")
                        evaluated.append({
                            "id": ids[i],
                            "generation": generation,
                            "genome": population[i],
        ...
        return _evo_components._orig_evaluate_population(fitness_function, population, ids, generation, all_parent_ids, log_dir_base, num_workers=1)
