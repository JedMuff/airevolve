...
## --- Job Array: 10 reps x 3 genotypes = 30 jobs ---
## Ordered so rep 0 runs for all genotypes first, then rep 1, etc.
## Index layout: 0=spherical/r0, 1=cppn/r0, 2=hybrid-cppn/r0,
##               3=spherical/r1, 4=cppn/r1, 5=hybrid-cppn/r1, ...
#SBATCH --array=0-29
...
# --- Derive genotype and repetition from array index ---
GENOTYPES=("spherical" "cppn" "hybrid-cppn")
GENO_IDX=$(( SLURM_ARRAY_TASK_ID % 3 ))
REP_IDX=$(( SLURM_ARRAY_TASK_ID / 3 ))
...
TMP_DIR="/tmp/lee_tuning_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
...
mkdir -p "$FINAL_DIR"
mv "$TMP_DIR" "$FINAL_DIR/lee_tuning_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
echo "Results saved to $FINAL_DIR/lee_tuning_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"

echo "Task $SLURM_ARRAY_TASK_ID ($GENOTYPE rep $REP_IDX) finished at: $(date)"
echo "done"
