...
## --- Job Configuration ---
#SBATCH --job-name=combined_hg_all
...
TMP_DIR="/tmp/combined_hg_${TASK}_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
...
# --- Run Combined Hover + Gate Evolution ---
cd "$PROJECT_DIR"
...
srun python3 experimentation/run_combined_hover_gate_evolution.py \
    ...
    --min-narms 6 \
    --max-narms 6 \
    --log-dir "$TMP_DIR/combined_hover_gate_runs" \
    $EXTRA_ARGS

# --- Move results to scratch ---
echo "Moving results from $TMP_DIR to $FINAL_DIR ..."
mkdir -p "$FINAL_DIR"
mv "$TMP_DIR" "$FINAL_DIR/combined_hg_${TASK}_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
echo "Results saved to $FINAL_DIR/combined_hg_${TASK}_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
