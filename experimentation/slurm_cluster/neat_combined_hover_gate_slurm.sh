...
## --- Job Configuration ---
#SBATCH --job-name=neat_combined_hg_all
...
TMP_DIR="/tmp/neat_combined_hg_${TASK}_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
...
srun python3 experimentation/run_neat_combined_hover_gate_evolution.py \
    ...
    --min-narms 6 \
    --max-narms 6 \
    --log-dir "$TMP_DIR/neat_combined_hover_gate_runs" \
    $EXTRA_ARGS

# --- Move results to scratch ---
echo "Moving results from $TMP_DIR to $FINAL_DIR ..."
mkdir -p "$FINAL_DIR"
mv "$TMP_DIR" "$FINAL_DIR/neat_combined_hg_${TASK}_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
echo "Results saved to $FINAL_DIR/neat_combined_hg_${TASK}_${GENOTYPE}_rep${REP_IDX}_${SLURM_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
