...
def plot_combined():
    dir1 = 'results_training/figure8/fig8_individual_1280'
    dir2 = 'results_training/figure8/fig8_individual_0042'
    ...
    x1 = np.arange(len(r1))
    plt.plot(x1, ma1, label='Best evolved drone', color='blue')
    plt.fill_between(x1, ma1 - std1, ma1 + std1, color='blue', alpha=0.2)

    x2 = np.arange(len(r2))
    plt.plot(x2, ma2, label='Most energy efficient drone', color='orange')
    ...
    print(f"Max reward for Individual 1280: {np.max(r1):.2f}")
    print(f"Max reward for Individual 0042: {np.max(r2):.2f}")
