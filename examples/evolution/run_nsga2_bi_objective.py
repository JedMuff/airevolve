"""Example: NSGA-II Bi-Objective Evolution (From the Research Paper).
...
Hardware target: CPU training with parallel multiprocessing.
  - Workers run parallel environments for PPO.
  - Thread-caps BLAS to 1 thread per worker.

Example
-------
  # Run a small-scale test of the NSGA-II evolution:
  python examples/evolution/run_nsga2_bi_objective.py \
      --population-size 16 --generations 8 --training-timesteps 500000 \
      --num-workers 8 --results-dir ./test_results

  # Reproduce the exact paper setup:
  python examples/evolution/run_nsga2_bi_objective.py \
      --population-size 24 --num-mutate 32 --generations 40 \
      --training-timesteps 10000000 --gate-cfg figure8 \
      --num-workers 24
"""
